...
Log.Logger = new LoggerConfiguration()
    ...
    .WriteTo.File("D:\\MessageMicroserviceLogging.txt")
    .CreateLogger();
...
builder.Services.AddSwaggerGen(options =>
{
    options.SwaggerDoc("v1", new OpenApiInfo()
    {
        Version = "v1",
        Title = "Message API",
        Description = "Пример MessageMicroservice",
    });
    
    var basePath = AppContext.BaseDirectory;
    var xmlPath = Path.Combine(basePath, "MessageMicroservice.xml");
...
builder.Services.AddSingleton<ChatIdProducer>();
