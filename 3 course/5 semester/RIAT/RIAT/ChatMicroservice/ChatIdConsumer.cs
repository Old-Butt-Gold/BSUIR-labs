...
public class ChatIdConsumer : BackgroundService
{
    ...
    public ChatIdConsumer(IServiceProvider serviceProvider, IConfiguration configuration)
    {
        _serviceProvider = serviceProvider;
        _configuration = configuration;
        
        // Setting up the RabbitMQ factory from the configuration
        _queueRequest = _configuration["RabbitMQ:RequestQueueChatId"]!;
        _queueResponse = _configuration["RabbitMQ:ResponseQueueChatId"]!; 
    ...
    protected override Task ExecuteAsync(CancellationToken stoppingToken)
    {
        ...
        consumer.Received += async (model, ea) =>
        {
            ...
            var chatName = (string)request.ChatName;
            ...
            var chat = await context.Chats.FirstOrDefaultAsync(c => c.ChatName == chatName, cancellationToken: stoppingToken);
            var chatId = chat?.ChatId ?? Guid.Empty;

            // Acknowledge the receipt of the message
            _channel.BasicAck(deliveryTag: ea.DeliveryTag, multiple: false);
            
            // Prepare the response message
            var response = Encoding.UTF8.GetBytes(chatId.ToString());
