...
public class UserIdProducer : IDisposable
{
    ...
    // Конструктор: создаем подключение и канал RabbitMQ
    public UserIdProducer(IConfiguration configuration)
    {
        _configuration = configuration;
        _factory = _configuration.GetSection("RabbitMQ").Get<ConnectionFactory>()!;
        _requestQueue = _configuration["RabbitMQ:RequestQueueUId"]!;
        _replyQueue = _configuration["RabbitMQ:ResponseQueueUId"]!;
    ...
    // This method requests the UserId by Username from the User Microservice
