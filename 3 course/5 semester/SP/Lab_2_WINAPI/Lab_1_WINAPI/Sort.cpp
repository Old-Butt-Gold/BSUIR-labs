...
void LoadFilesData(HWND hListView, std::vector<FileData>& gFilesVector, const std::wstring directoryPath, std::vector<FileData>& files, std::unordered_map<std::wstring, int>& gNameCount,
    ...
    do {
        if (data.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY) {
            if (wcscmp(data.cFileName, L".") != 0 && wcscmp(data.cFileName, L"..") != 0) {
                ...
                LoadFilesData(hListView, gFilesVector, newDirPath, files, gNameCount, errorLogFile);
            }
        }
        else {
            FileData fileData;
