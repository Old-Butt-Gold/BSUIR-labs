...
Table::Table(HWND parent, int rows, int cols, HFONT hHeaderFont, HFONT hCellFont)
    ...
    this->hFileNamesFont = hCellFont;
    this->hFileSizesFont = hCellFont;
    ...
    this->hwnd = CreateWindowEx(0, WC_CUSTOM_LIST_VIEW, L"",
        WS_CHILD | LVS_REPORT,
        0, 0, SCREEN_WIDTH - 2, SCREEN_HEIGHT - 25,
...
void Table::OnPaint() {
    ...
    // ��������� ����������
    RECT headerRect = { MARGIN_LEFT, 0, clientRect.right - 2, this->header->GetColumnHeight() };
...
LRESULT CALLBACK Table::CustomListWndProc(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam) {
    ...
    case WM_SIZE:
    {
            listView->OnSize();
            RECT windowRect;
            GetWindowRect(listView->hwndParent, &windowRect);
            
            int newWidth = LOWORD(lParam);
            int newHeight = HIWORD(lParam);

            int originalColumnHeight = COLUMN_HEADER_HEIGHT; // Изначальная высота колонок
            int originalItemHeight = ITEM_HEIGHT;            // Изначальная высота элементов

            int newColumnHeight = originalColumnHeight * newHeight / SCREEN_HEIGHT;
            int newItemHeight = originalItemHeight * newHeight / SCREEN_HEIGHT;

            listView->GetHeaderClass()->SetColumnHeight(newColumnHeight);
            listView->SetItemHeight(newItemHeight + 1);

            listView->GetHeaderClass()->Resize(newWidth);
    }
    return 0;
